...
- Added a fail-closed CryptoTrader PR-create guard on generic Hermes tool surfaces. The dedicated Researcher lane remains the only approved path that may open or reuse a `m0nklabs/cryptotrader` PR; ad-hoc `terminal` and `execute_code` attempts to run `gh pr create` or direct GitHub PR-create API calls are now blocked before execution. Synced the Kyber role/architecture docs so they no longer imply the Coder opens PRs.
- Added Harbor as a first-class Kyber evaluation lane. Installed the live Harbor CLI from the local `/home/flip/harbor` checkout, validated the Claude Code / Hermes / Aider installed-agent slice in Harbor, added `scripts/harbor_eval.sh` for direct local benchmark runs, documented the Harbor wrapper and env contract in `docs/HARBOR_EVALS.md` and `docs/CONFIGURATION.md`, and recorded Harbor in the workspace inventory/policy.
- Added closed-without-merge PR recovery protocol across all Aider agent roles and the Hermes KANBAN_GUIDANCE prompt. When a PR is closed without merge, Reviewers (Tier1 and Tier2) refuse to review and emit `next_action: researcher-replan`, the Coder refuses to push to the dead branch, the Hermes worker checks `gh pr list --state all` before starting and creates a fresh branch from current master with a `Supersedes #NNN` link in the PR body, and the Researcher analyzes why the prior attempt failed before planning a tighter-scope fresh approach. Documented the full handoff sequence in `docs/AIDER_AGENT_ROLES.md`.
- Achieved the first fully autonomous CryptoTrader pipeline: issue #334 → PR #335 in ~4 minutes without operator intervention. The Kanban sync picked up the issue, auto-decomposed it into 3 subtasks (implement → test → verify), spawned a worker that studied `/health` patterns, wrote `/version` endpoint + 4 tests, committed with a `Co-Authored-By` trailer (model `qwen3.6-35b-uncensored` visible), pushed a feature branch, and opened a clean PR. Captured two hygiene failures (worker committing to master, `Closes #slug` instead of `Closes #NNN`) and hardened the KANBAN_GUIDANCE prompt accordingly. Added iron-strong system prompts per Aider agent role (`~/.hermes/prompts/aider/{researcher,coder,reviewer_tier1,reviewer_tier2}.md`) and documented agent responsibilities in `docs/AIDER_AGENT_ROLES.md`.
- Added runtime retry knobs to the CryptoTrader Aider reviewer loop (`cryptotrader_pr_aider_reviewer_loop.py`): `AIDER_REVIEW_MAX_RETRIES` (default 3), `AIDER_REVIEW_RETRY_BACKOFF_BASE` (2.0s), `AIDER_REVIEW_RETRY_BACKOFF_JITTER` (0.5s), and `AIDER_REVIEW_RATE_LIMIT_COOLDOWN_SECONDS` (5s). The loop now applies exponential backoff with jitter on HTTP 429 rate limits and 5xx server errors, maintains the existing fallback logic for invalid model IDs, and enforces a cooldown between invocations to prevent rate-limit stacking.
- Completed 4-phase documentation hardening series. Added `TESTING.md` (runbooks for all 8 test scripts with pass/fail criteria and known propagation bugs), `INCIDENT_RESPONSE.md` (P1–P4 severity classification plus runbooks for gateway crash loops, stuck issues, OpenRouter outages, GitHub rate limits, SQLite corruption, Guardian unreachable), and `BACKUP.md` (automated backup script with 7-day rotation, per-component restore procedures, full disaster recovery rebuild steps). Hardened `OPERATOR_GUIDE.md` with a startup checklist, `gh` CLI primer, SQLite schema overview, and escalation path. Added `CONFIGURATION.md` as a single-source reference for all env vars across Claude Code launchers, Hermes Gateway, Guardian, and CryptoTrader. Enforced the Guardian boundary: Kyber docs call only the `:11434` proxy and never discuss GPU/VRAM/llama-server tuning. Indexed all operations docs under a new "Operations" section in `docs/index.md`.
- Formalized the Kyber -> Hermes -> GitHub -> target-repo hierarchy in the workspace docs, documented the three-phase program (framework stabilization, CryptoTrader success, multi-repo scaling), and aligned the roadmap/TODO narrative around Hermes remaining a generic multi-repo engine.
- Switched the live CryptoTrader PR flow into a temporary manual gatekeeper mode: Mergify no longer merges, local CryptoTrader/Hermes helper lanes no longer call GitHub merge directly, and merge-ready PRs now emit a prominent GitHub comment plus Telegram notification for operator review and manual merge.
- Aligned Kyber operator docs and smoke-test guidance to the isolated `./cryptotrader_hermes` sandbox on the 51000/51176/51787 port range, and validated the installed `ct-backend-hermes.service` / `ct-frontend-hermes.service` start-serve-stop path under systemd.
- Recovered the latest CryptoTrader protected-master local drift into `m0nklabs/cryptotrader` PR #325 (`hermes/recover-local-drift-20260601`) and restored the local CryptoTrader `master` checkout to `origin/master`.
- Added Hermes PR-manager tier routing: Tier1 OpenRouter `deepseek-v4-flash`, Tier2 Guardian `gemma4-26b-a4b`, Tier3 OpenRouter `deepseek-v4-pro`, Copilot/code-quality suggestion counting, coder-tier escalation, and exact `Ready for merge` audit comments after Tier2/highest clean review.
- Added explicit downstream PR discipline for Kyber-managed projects: Hermes/Aider work outside the KyberM0nk framework scope must stay on an issue branch, be submitted through a GitHub PR, and link the source issue; the Hermes issue lane now checks branch and uncommitted-drift state after local coder execution before push/PR creation.
- Tightened the live CryptoTrader Aider reviewer loop so Tier1 reports only concrete blocking findings, treats harmless npm lockfile metadata churn as non-blocking, posts findings through GitHub inline review threads with suggestion blocks, and avoids body-only `review_findings` fallbacks.
- Temporarily disabled PR-manager auto-merge defaults in Hermes issue resolution plus the live CryptoTrader PR result sync and simple PR autopilot scripts; merge-ready PRs now require operator action unless `HERMES_ISSUE_AUTO_MERGE_ENABLED=1`, `PR_MANAGER_AUTO_MERGE_ENABLED=1`, or `PR_AUTOPILOT_AUTO_MERGE_ENABLED=1` is explicitly set.
- Recorded the Hermes Kanban task audit and done-state closure implementation (Hermes local changes pending commit/push), which lets `/issue` runs carry `--kanban-task` and `--kanban-board`, writes task events/comments for claim, PR open/reuse, review routing, merge, issue closure, cancellation, and failure, and completes linked Kanban tasks after automatic PR merge and issue closure.
