...
The sandbox provides these Windows validation helpers:
...
- `windows-pwsh '<PowerShell command>'` for simple Windows validation only

Use `newnexus-windows-build` for normal Windows sync/build validation. Do not use `windows-pwsh` to run `Get-Content`, `Set-Content`, `Add-Content`, or any source mutation against `J:\Unreal Projects\NewNexus`; the wrapper blocks those commands. Inspect and edit source files directly in `/a0/usr/projects/newnexus`.
...
- Do not treat `windows-pwsh` as a source editing tool; it is only a Windows validation helper. Prefer `newnexus-windows-build` for NewNexus status, pull, query-targets, and build.
