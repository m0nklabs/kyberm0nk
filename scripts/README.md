...
- `crewai_main_quest_control.py`: shared control path for the tracked CrewAI main quest with `run`, `start`, `restart`, `status`, `stop`, `get-inputs`, and `set-inputs` modes, including persisted `repo_write_mode` and `github_target_branch` safety inputs for live pilots.
