...
| 11 | Automatic merge and issue closure | `ready_for_merge` now closes PR, issue, and linked Kanban task state when a Kanban task id is provided. | Completes zero-manual issue-to-merge loop. | Medium | Medium | Pre-merge blocker; CI status checks; permissions | Hermes | Implemented in Hermes fork commit `bf542e744`: current-head `ready_for_merge` PRs merge automatically only when the live PR is open, non-draft, still on the reviewed head, and `mergeStateStatus=CLEAN`; linked issues are closed and merge/closure audit comments are written. Kanban task done-state closure implemented in Hermes local changes pending commit/push. | 5 | 3 | 5 | 2 | 15 | 3 |
| 12 | Audit comments at lifecycle points | Claim, PR, review, fix, merge, closure, cancellation, and failure transitions are now uniformly auditable across GitHub and linked Kanban tasks. | Improves observability and forensic debugging. | Medium | Low | Hermes state transitions | Hermes | Implemented for GitHub issue/PR transitions in Hermes fork commit `863aad702`; automatic merge and issue-closure audit comments implemented in Hermes fork commit `bf542e744`; linked Kanban task audit events/comments implemented in Hermes local changes pending commit/push. | 4 | 3 | 4 | 3 | 14 | 2 |
...
| 16 | Expand reviewer output into anchored inline comments | Review findings may be hard to apply when only summary comments exist. | Improves review quality and fix precision. | Medium | Medium | Diff position mapping; GitHub review API | Hermes/Kyber | Implemented in Hermes local changes pending commit/push: reviewer findings prefer inline review comments, body-only findings are not routed to coder, and Copilot/code-quality inline comments count as actionable suggestion signals. | 4 | 2 | 3 | 2 | 11 | 3 |
| 17 | Cloud escalation gates | Cloud review/escalation should trigger only on repeated failures or risky diffs. | Controls cost while preserving quality. | Medium | Low | Review state and diff-risk classifier | Kyber | Implemented in Hermes local changes pending commit/push: Tier1 uses OpenRouter flash, Tier2 uses Guardian local review, Tier3 OpenRouter pro is reserved for inconclusive/escalated review states, and Tier2/highest clean emits exact `Ready for merge` audit text. | 3 | 2 | 3 | 3 | 11 | 3 |
...
- Pre-merge blocker.
- Automatic merge, linked issue closure, and linked Kanban task done-state closure. (PR merge and linked issue closure implemented in Hermes fork commit `bf542e744`; Kanban done-state closure implemented in Hermes local changes pending commit/push.)
...
7. Duplicate suppression and per-repo allowlists/cancellation controls. (Master Epic sub-issue dedupe implemented in Hermes fork commit `23d979e61`; repo allowlist implemented in fork commit `97667b70d`; cancellation controls remain.)
...
1. Automatic merge, issue closure, and linked Kanban task closure. (PR merge and linked issue closure implemented in Hermes fork commit `bf542e744`; Kanban done-state closure implemented in Hermes local changes pending commit/push.)
2. Priority/capability metadata in `issue_runs`.
3. Anchored inline reviewer comments. (Implemented in Hermes local changes pending commit/push, including Copilot/code-quality suggestion counting.)
4. Cloud escalation gates. (Implemented in Hermes local changes pending commit/push as Tier1/Tier2/Tier3 review routing.)
...
- Routine clean PRs merge, close linked issues, record audit comments automatically, and complete linked Kanban tasks when the run carries a Kanban task id.
