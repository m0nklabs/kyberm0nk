...
| 14 | Per-repo allowlists and cancellation controls | Issue automation needs clearer repo boundaries and operator stop controls. | Reduces accidental cross-repo or unwanted execution. | Medium | Medium | Gateway config; issue lane | Hermes | Repo allowlist implemented in Hermes fork commit `97667b70d`: `/issue` defaults to `m0nklabs/cryptotrader` and blocks unconfigured repos before issue loading. Operator cancellation controls remain next. | 4 | 3 | 4 | 3 | 14 | 2 |
